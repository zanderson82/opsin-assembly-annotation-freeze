...
echo "=== Starting OPSIN gene analysis pipeline ==="
...
process_haplotype() {
    ...
    python3 $OUTPUT_DIR/analyze_opsin_exon5.py ${OUTPUT_PREFIX}.gff $SAMPLE_ID $HAP_NAME > ${OUTPUT_PREFIX}_analysis.txt
    
    # Return success
    return 0
}

# Create the Python analysis script focused solely on exon5 annotations
cat > $OUTPUT_DIR/analyze_opsin_exon5.py << 'EOF'
#!/usr/bin/env python3
import re
import sys
...
def analyze_opsin_exon5(gff_file, sample_id, hap_name):
    ...
    if not annotations:
        ...
        return 0, 0, "Unknown (No annotations)"
    ...
    # Find LCR positions
    lcr_positions = [a for a in annotations if a['type'] == 'LCR']
    
    if not lcr_positions:
        print("WARNING: No LCR annotations found. Cannot determine array structure.")
        return lw_exon5_count, mw_exon5_count, "Unknown (No LCR found)"
    
    # Get only the first LCR (first in the sorted order)
    first_lcr = lcr_positions[0]
    lcr_position = first_lcr['position']
    
    if is_reverse_orientation:
        print(f"First LCR found at position {lcr_position} (reverse orientation)")
    else:
        print(f"First LCR found at position {lcr_position} (forward orientation)")
    
    # Get all exon5 annotations, regardless of position relative to LCR
    ...
    if len(all_exon5) == 0:
        ...
        return lw_exon5_count, mw_exon5_count, "Incomplete (no exon5 annotations found)"
    
    # Sort all exon5 by position according to orientation
    if is_reverse_orientation:
    ...
    if len(reordered_exon5) >= 1:
        ...
        return lw_exon5_count, mw_exon5_count, f"{array_structure}{orientation_indicator}"
    else:
...
chmod +x $OUTPUT_DIR/analyze_opsin_exon5.py

# Create a summary file with array structure information
if [[ "$2" == *"with-hg-size"* ]]; then 
    SUMMARY_FILE="${OUTPUT_DIR}/opsin_array_summary-with-hg-size.tsv"
else
    SUMMARY_FILE="${OUTPUT_DIR}/opsin_array_summary-without-hg-size.tsv"
fi
if [ ! -s "$SUMMARY_FILE" ]; then
    echo -e "SampleID\tSex\tHaplotype\tOPN1LW_copies\tOPN1MW_copies\tTotal_OPSIN_copies\tArray_Structure" > "$SUMMARY_FILE"
fi
...
while IFS=$'\t' read -r SAMPLE_ID SEX EXTRA_COLS; do
    ...
    if [ -z "$SAMPLE_DIR" ]; then
        ...
        echo -e "$SAMPLE_ID\t$SEX\tNA\tNA\tNA\tNA\tNA" >> $SUMMARY_FILE
        continue
    fi
    ...
    if [[ "$SEX" == "XY" ]]; then
        ...
        if [ -f "$ASSEMBLY" ]; then
            ...
            # Add to summary
            echo -e "$SAMPLE_ID\t$SEX\tprimary\t$LW_COUNT\t$MW_COUNT\t$TOTAL_COUNT\tstructure: [LCR]-$ARRAY_STRUCTURE" >> $SUMMARY_FILE
        else
            echo "ERROR: Assembly file not found for $SAMPLE_ID"
            echo -e "$SAMPLE_ID\t$SEX\tprimary\tNA\tNA\tNA\tNA" >> $SUMMARY_FILE
        fi
    elif [[ "$SEX" == "XX" ]]; then
        ...
        if [ -f "$HAP1_ASSEMBLY" ]; then
            ...
            # Add to summary
            echo -e "$SAMPLE_ID\t$SEX\thap1\t$LW_COUNT\t$MW_COUNT\t$TOTAL_COUNT\tstructure: [LCR]-$ARRAY_STRUCTURE" >> $SUMMARY_FILE
        else
            echo "ERROR: Haplotype 1 assembly file not found for $SAMPLE_ID"
            echo -e "$SAMPLE_ID\t$SEX\thap1\tNA\tNA\tNA\tNA" >> $SUMMARY_FILE
        fi
        ...
        if [ -f "$HAP2_ASSEMBLY" ]; then
            ...
            # Add to summary
            echo -e "$SAMPLE_ID\t$SEX\thap2\t$LW_COUNT\t$MW_COUNT\t$TOTAL_COUNT\tstructure: [LCR]-$ARRAY_STRUCTURE" >> $SUMMARY_FILE
        else
            echo "ERROR: Haplotype 2 assembly file not found for $SAMPLE_ID"
            echo -e "$SAMPLE_ID\t$SEX\thap2\tNA\tNA\tNA\tNA" >> $SUMMARY_FILE
        fi
    else
